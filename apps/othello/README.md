...
- Export JSON includes all branches. Import caps input at 1MB/1,000 events and replays each branch to validate legality; imported games receive a new ID and count as practice. Only canonical record fields are retained. After deletion, the banner offers 「元に戻す」 or 「削除したままにする」. Reload also drops the offer. Records are local to this origin; use one active tab per game.
