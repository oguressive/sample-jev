...
The API binds only to loopback port 8788. Vite proxies `/api` to it. Public hosting is intentionally not configured: add authentication, a same-origin reverse proxy, HTTPS and a persistent per-user spending/rate budget before exposing this paid endpoint. The local process allows 2 concurrent requests and 120 evaluations/minute globally, a 16KB request body and no SDK retries. 120 is one maximum game with hints (at most 60 placed disks, one evaluation each) plus a review of every human move in the same minute. Passes do not add an evaluation. Each upstream call may take 60s; because batches run sequentially, an evaluation's deadline is `batches × 60s + 5s` (1 batch per 8 legal moves), and the browser waits that plus a possible 45s warmup.

Jev can cold-start (about 20s on the first call). When the UI loads and the API is configured, it posts `/api/warmup`. The server treats this as a hint: it sends one fixed, tiny question (the browser cannot choose its content), shares an in-flight warmup, skips it within 5 minutes of any successful Jev call, waits 60s after a failure, and caps warmups at 12/hour, so reopening TOP cannot multiply cost. An evaluation arriving during a warmup waits for it instead of starting a second cold request. The TypeSafe upstream URL is fixed. Secrets are never requested from the browser or returned in error messages. `VITE_` variables must never contain a key.
...
- Export JSON includes all branches. Import caps input at 1MB/1,000 events and replays each branch to validate legality; imported games receive a new ID and count as practice. Only canonical record fields are retained. After deletion, the banner offers 「元に戻す」 until another deletion or reload. Records are local to this origin; use one active tab per game.
...
Each request batches at most 8 moves (24 questions) per upstream call, at most 8 calls for a 64-square board, with an overall deadline. CPU play costs calls every turn; hints add analysis calls. Review can evaluate up to all human turns, so the UI shows its maximum before starting. Actual model, tokens, call count, elapsed milliseconds and question/formula versions appear in the evaluation details. Candidate facts give each side's mobility on the resulting board (`actingPlayerMobility`, `opponentMobility`) and explicit forced passes; changing these facts bumps the question version, so cached analyses are not reused across prompt versions. Live speed, cost and strength require a separate explicitly authorized experiment.
